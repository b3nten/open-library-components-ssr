...
router.get(routes.home, async () => {
	...
	const books = (await import("../../public/data/books.json")).default

	const shuffled = [...books].sort(() => Math.random() - 0.5);
	const selectedBooks = shuffled.slice(0, 8);

	const bookCovers = selectedBooks
		.map((book) => {
			const buttonLabel =
	...
	return renderHtml(pageShell({
		head: page.head,
		body: page.body.replace("<!-- BOOK COVERS -->", bookCovers)
	}));
});

router.get(routes.components, async () =>
	renderHtml(pageShell(await getPage("components"))),
);

router.get(routes.forms, async () =>
	renderHtml(pageShell(await getPage("forms"))),
);

router.get(routes.signup, async () =>
	renderHtml(pageShell(await getPage("signup"))),
);

router.get(routes.dropdownTest, async () =>
	renderHtml(pageShell(await getPage("dropdown-test"))),
);
